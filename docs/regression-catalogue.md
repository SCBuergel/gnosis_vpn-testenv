...
The suite lives in `scripts/suite/`: one `tNN.sh` per catalogue entry, a shared `lib.sh`, the runner `run.sh`, the version-matrix driver `matrix.sh`, the probes in `probes/` (`relprobe.py` for bidirectional echo calls, `streamprobe.py` for one-way paced streams, `callprobe.py` for the forced-reconnect call) and `node-sampler.py` for node metrics. `just suite` runs the whole thing, `just test tNN` runs one entry. Everything talks to the live stack through `docker exec gnosis_vpn-client gnosis_vpn-ctl …`, the localcluster's `status` JSON and the nodes' REST and `/metrics` endpoints. Traffic goes to the in-cluster target container, which serves the sized download and upload endpoint on `:8899`, a UDP echo on `:8901`, a paced stream server on `:8902` and the two-ended call server on `:8903`.

Conventions, shared with the older tooling in `scripts/`:

- `set -euo pipefail`, `export LC_ALL=C`, a header comment that says what the test asserts, `--help`.
- Every knob is an env var with a default, written `: "${VAR:=default}"` at the top of the script; `q DEFAULT FAST` picks the `--fast` value. A per-test knob is passed from the runner as `T<NN>_<VAR>`.
- Each script declares its kind with `suite_kind gate|diagnostic|runbook` and reports through `verdict` (PASS, WARN, FAIL, SKIP), `record` (RECORDED) and `xfail`. A FAIL from a non-gate is downgraded to WARN, so only a gate can redden a run.
- Every measurement is also a JSON line in `rows.jsonl`, and every verdict a line in `verdicts.jsonl`, so two runs diff cleanly.
- The library has offline unit tests in `scripts/tests/suite-lib.bats`.
...
**Relative scoring.** On a single-host stack the exit, relays, client and target share one CPU, so every throughput or latency number is a host artifact. Those are scored as a **delta against the last stored value of the same metric on the same stack**, normally the previous run, with the tolerance taken from T03-repeatability-baseline's measured band. A named reference cell (`--ref-cell`) exists only for the explicit A/B path of `matrix.sh`, for a newly added test with no history. Absolute pass/fail is reserved for discrete assertions: zero decapsulation errors, zero `DecapStalled`, one datagram per read, channel membership, the shaper present or absent.

**T03-repeatability-baseline is a prerequisite, not advice.** A stack with no T03-repeatability-baseline record has no defensible band, so `run.sh` still runs and records everything but *refuses to score* the host-dependent gates, and says so in the run header. T03-repeatability-baseline runs third in every run, so the band exists from the first full run on; to create it alone, run `just suite --only t01,t02,t03`.
...
| Kind | Tests |
| --- | --- |
| **gate** | T01-topology-preconditions (incl. the client channel pin), T02-build-provenance, T04-fixed-throughput (errors absolute, throughput delta), T05-loaded-latency (delta-gated; must run early), T06-realtime-udp (all three arms gated), T07-cold-start, T08-relay-attribution (membership; split gated only at equal latency), T09-impairment-ladder (equal-latency rungs), T10-forced-reconnect, T11-capability-matrix, T12-balancer-sweep (incl. the masking cell), T13-mtu-sweep (XFAIL until `hoprnet#8392`), T22-concurrent-clients, T23-sustained-soak, T24-sustained-upload |
...
**Method.** Repeat one unchanged cell (T04-fixed-throughput, warm) `N` times back to back, with no version, config or topology change, and report median, spread, min/max and the run-to-run distribution. Derive the minimum detectable effect at the suite's chosen n. Re-run per stack, since the answer is a property of the stack.
...
**Method.** Three arms, each on its own session. A bidirectional echo call (`relprobe.py` against the target's `:8901`) at `ECHO_RATE` for `ECHO_DUR`, then an upload-only and a download-only paced stream (`streamprobe.py` against `:8902`) at `STREAM_RATE` for `STREAM_DUR`. Each probe reports loss, the delay distribution above its minimum, stalls, rebinds and outage time.
...
**State on the current stack.** Every equal rung has failed in three runs on hoprd 4.1.3 with client 0.96.3 (fullrun5, fullrun6 and rerun1, 2026-09-20): the transfers complete at 10-13 Mbit/s, but the download stream that follows them on the same session shows 9 to 54 reassembly failures and 0.7 to 71 % loss, and the 50 ms rungs reconnect once after three tunnel-ping timeouts. T06-realtime-udp's identical stream on a fresh session records zero reassembly failures and 0.2 % loss, so the difference is the state the bulk transfers leave on the session, not the impairment. The mechanism is open; until it is understood the test reports that finding rather than latency tolerance.
...
**Parameters.** `DUR`=3600 s (`--fast` 600, `--very-fast` 60), `INTERVAL`=300 s (`--very-fast` 30), `LOG_MB_MIN_MAX`=200 MB/min; a 1.5 Mbit/s echo call for `DUR` plus one `BYTES` download and upload every `INTERVAL`. The session is covered by `deadman_cover DUR`: fullrun5 ran the hour under the 900 s deadman default, the client was disconnected at +15 min, every later transfer read 0 Mbit/s, the call delivered 24 % (= 900/3600) and the verdict was PASS, because a deadman disconnect is not a reconnect. That was the "soak death" of 2026-09-20 in the suite; it was the harness.
...
**Pass criteria.** Per MTU: PASS iff stream loss < 5 % and `reconnects` = 0 and the client's `hopr_packet_rejected_count{reason="undecodable"}` grew by fewer than 50. A missing end-of-stream report from the server is an UNMEASURED FAIL that says so and names the second the tunnel interface went down, if the probe saw it go down. The session is covered by `deadman_cover DUR`: fullrun5 ran the 900 s arms under the 900 s deadman default, the client was disconnected ~30 s before the sender finished, and both arms printed a blank loss.
...
**testenv notes.** Reads the client telemetry with `docker exec … gnosis_vpn-ctl -o json telemetry` (`telemetry_metric` in `lib.sh`); scraping it into VictoriaMetrics (extension 8) is still open.
...
**Order matters.** T03-repeatability-baseline runs third so the run is scored against a band measured on this stack now. T05-loaded-latency runs fifth, right after the T04-fixed-throughput reference, because loaded-RTT numbers taken after an hour of other tests measure accumulated host load rather than the stack, and T06-realtime-udp runs sixth for the same reason. Everything else is ordered cheap to expensive so a broken stack fails early, and a T01-topology-preconditions failure aborts the run.

**Every test lives in exactly one list in `run.sh`**, `TESTS` or `RUNBOOK`. A test in neither is dead code that still looks maintained: T19-background-load and T20-fault-injection sat in no list from the restructure until 2026-09-17, so contending-load and relay-fault coverage silently never ran.

**Shortening a run.** `--fast` substitutes the catalogue's own shorter durations (the values in parentheses in each entry) and never drops a sustained arm below 90 s. `--very-fast` cuts every long step to the shortest setting that still exercises its mechanism, 15 s arms, and therefore cannot see anything that needs a long session, such as a reconnect cycle on the order of a minute; never conclude "healthy" from it. `--only tNN,tNN` and `--skip tNN` are for working on one test. A per-test knob is `T<NN>_<VAR>=…` in the environment (`T23_DUR=150`), and it overrides `--very-fast`.
