...
**Common parameters** (from `lib.sh`; a `--fast` or `--very-fast` run substitutes the value in parentheses): `BYTES`=10000000 (`--fast` 5000000, `--very-fast` 2000000), `CAP`=90 s (`--fast` 60, `--very-fast` 30), `REPS`=3 (`--fast` 2, `--very-fast` 1), `SURB_RAMP_WAIT`=25 s (the floor on every post-connect idle unless the test sets `RAMP_WAIT_OPT_OUT=1`), `CONNECT_TIMEOUT`=240 s, `DEADMAN`=900 s (a test whose session must last longer calls `deadman_cover DUR`, which raises it to DUR + `SURB_RAMP_WAIT` + 120 s; T23-sustained-soak and T24-sustained-upload do), `DEST`=node-0, `CLUSTER_SIZE`=3, `BAND_MAX_PCT`=50. **`--fast` rule:** a quick value never drops a sustained-flow arm below 90 s, so that anything on the timescale of the client's liveness watchdog (3 pings × 10 s interval + 15 s timeout ≈ 85 s) is still inside every arm; `--very-fast` (15 s arms) is the mode that trades that visibility for time. **Delta scoring** (`score_delta`): a host-dependent number is compared with the last stored value of that metric from a run in the same mode (full, `--fast` and `--very-fast` keep separate histories, because a 2 MB smoke transfer is not a 10 MB one: a `--very-fast` T13-mtu-sweep once failed by 30 % against the full run's number); it FAILs when the change exceeds the band `mde_pct` written by T03-repeatability-baseline (capped at `BAND_MAX_PCT`=50 %) in the worse direction, and is RECORDED, not scored, on the first observation of a metric or when the stack has no band. Every `Pass criteria` below states what the script enforces; anything the original design asked for but the script does not assert is marked "recorded, not asserted".
