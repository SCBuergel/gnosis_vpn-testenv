...
The suite lives in `scripts/suite/`: one pytest module `tests/test_tNN_<name>.py` per catalogue entry, the library `suitelib/`, the pytest glue `conftest.py`, the version-matrix driver `matrix.py`, the probes in `probes/` (`relprobe.py` for bidirectional echo calls, `streamprobe.py` for one-way paced streams, `callprobe.py` for the forced-reconnect call) and `node-sampler.py` for node metrics. `just suite` runs the whole thing, `just test tNN` runs one entry. Everything talks to the live stack through `docker exec gnosis_vpn-client gnosis_vpn-ctl …`, the localcluster's `status` JSON and the nodes' REST and `/metrics` endpoints. Traffic goes to the in-cluster target container, which serves the sized download and upload endpoint on `:8899`, a UDP echo on `:8901`, a paced stream server on `:8902` and the two-ended call server on `:8903`.
...
**Common parameters** (from `suitelib/config.py`; a `--fast` or `--very-fast` run substitutes the value in parentheses): `BYTES`=10000000 (`--fast` 5000000, `--very-fast` 2000000), `CAP`=90 s (`--fast` 60, `--very-fast` 30), `REPS`=3 (`--fast` 2, `--very-fast` 1), `SURB_RAMP_WAIT`=25 s (the floor on every post-connect idle unless the test connects with `ramp_wait_opt_out=True`), `CONNECT_TIMEOUT`=240 s, `DEADMAN`=900 s (a test whose session must last longer calls `client.deadman_cover(DUR)`, which raises it to DUR + `SURB_RAMP_WAIT` + 120 s; T23-sustained-soak and T24-sustained-upload do), `DEST`=node-0, `CLUSTER_SIZE`=3. **`--fast` rule:** a quick value never drops a sustained-flow arm below 90 s, so that anything on the timescale of the client's liveness watchdog (3 pings × 10 s interval + 15 s timeout ≈ 85 s) is still inside every arm; `--very-fast` (15 s arms) is the mode that trades that visibility for time. Every `Pass criteria` below states what the script enforces; anything the original design asked for but the script does not assert is marked "recorded, not asserted".
