...
def test_build_provenance(cfg, run, client, checks, knobs):
    ...
    d = {"client_version": cv, "client_protocol": cp, "hoprd_version": hv, "hoprd_protocol": hp, "server_version": sv,
    ...
    print(json.dumps(d))
    checks.row(client=cv, client_protocol=cp, hoprd=hv, hoprd_protocol=hp, server=sv)
    ...
    if cp and hp and cp == hp:
        checks.passed(f"client {cv} ({cp}), hoprd {hv} ({hp}), server {sv}")
    elif cp and hp:
        checks.failed(f"protocol id mismatch: client {cv} speaks {cp}, hoprd {hv} speaks {hp}; a HOPR packet's frame size "
                      f"is fixed, so mismatched versions misparse rather than refuse to connect")
    else:
        checks.warn(f"could not read a protocol id: client '{cp}' hoprd '{hp}' (client {cv}, hoprd {hv}, server {sv})")
