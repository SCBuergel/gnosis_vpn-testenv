...
# Pass: no reconnect, no unbounded RSS growth (last < 2× first), log rate < LOG_MB_MIN_MAX MB/min (default 20).
...
python3 -c "import sys; sys.exit(0 if $(json_get "$e" reconnects)==0 and ${rss1:-0} < 2*max(${rss0:-1},1)+200000 and $lograte < $LOG_MB_MIN_MAX else 1)" && verdict T23-sustained-soak PASS "$msg" || verdict T23-sustained-soak FAIL "$msg"
exit $SUITE_FAILED
